...
@RestController
@RequestMapping("/reimmaan")
public class ReimManagController {
    ...
    @Autowired
    public ReimManagController(ReimbursementServices reimbursementServices, RestTemplate restTemplate) {
    ...
    public ResponseEntity<?> updateStatus(@PathVariable Integer reqid, @RequestBody String status) throws URISyntaxException {
        ...
        if (statusUpdate.getStatus().equals("Denied")) {
            logger.error("A denied request must be sent to the Reimbursement Manager.");
        ...
        if (statusUpdate.getStatus().equals("Pending")) {
            logger.error("");
            return ResponseEntity.ok("Request is pending, please reassign the ");
        }
        if (status.equals("Denied")) {
            statusUpdate.setStatus("Denied");
            reimbursementServices.updateStatus(dto);
            logger.info("Reimbursement Manager has denied the Reimbursement");
        ...
        ResponseEntity<Object> responseEntity = restTemplate.postForEntity(url, Arrays.asList(statusUpdate.getEmpId().getEmployeeEmail(),
                statusUpdate.getAmount().toString(), statusUpdate.getStatus()), null);
